					...
					unsigned long uarg)
{
	...
	case HPROX_STRUCT_KVM_GET_OFFSET:
		...
		case HPROX_ARCH_PKVM_ENABLED:
			return offsetof(struct kvm, arch.pkvm.enabled);
		case HPROX_ARCH_PKVM_TEARDOWN_MC:
			return offsetof(struct kvm, arch.pkvm.stage2_teardown_mc);
		case HPROX_ARCH_PKVM_PVMFW_LOAD_ADDR:
			return offsetof(struct kvm, arch.pkvm.pvmfw_load_addr);
		default:
			return -EINVAL;
		}
	...
	case HPROX_STRUCT_KVM_VCPU_GET_OFFSET:
		...
		case HPROX_VCPU_MEMCACHE:
			return offsetof(struct kvm_vcpu, arch.stage2_mc);
		case HPROX_VCPU_HYP_REQS:
			return offsetof(struct kvm_vcpu, arch.hyp_reqs);
		default:
			return -EINVAL;
		}
					...
					unsigned long uarg)
{
	...
	if (minpages)
		topup_hyp_memcache(&mc, minpages, 0);
	else
