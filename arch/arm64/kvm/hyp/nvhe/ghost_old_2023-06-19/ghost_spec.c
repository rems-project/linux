...
struct ghost_state spec_handle_host_hcall(struct ghost_state *g) // pointer or value argument and result?
{
	DECLARE_REG(unsigned long, id, &g->host_ctxt, 0);
...
#pragma GCC diagnostic ignored "-Wunused-label" // not sure why the next lines trigger that
	...
	cpu_reg(&g->host_ctxt, 0) = SMCCC_RET_SUCCESS;
        return *g; // TODO construct new one

	inval:
	cpu_reg(&g->host_ctxt, 0) = SMCCC_RET_NOT_SUPPORTED;
        return *g; // TODO construct new one
}


struct ghost_state spec_handle_host_mem_abort(struct ghost_state *g)
{
	return *g; // TODO construct new one
}


struct ghost_state spec_BUG(struct ghost_state *g)
{
	return *g; // TODO construct new one
}
...
struct ghost_state spec_handle_trap(struct ghost_state *g)
	// pointer or struct arguments?  for more obvious correspondence to math, struct - but maybe too terrible for impl, and for idiomatic-readable-C?  But this code should never mutate them, in any case.
{
	switch (ESR_ELx_EC(g->sysregs[GHOST_ESR_EL2])) {
	case ESR_ELx_EC_HVC64:
		return spec_handle_host_hcall(g);
	// TODO
	//case ESR_ELx_EC_SMC64:
	//	handle_host_smc(host_ctxt);
	//	break;
	...
	case ESR_ELx_EC_IABT_LOW:
	case ESR_ELx_EC_DABT_LOW:
		return spec_handle_host_mem_abort(g);
	default:
		return spec_BUG(g);
	}
}
