...
#ifdef CONFIG_NVHE_GHOST_SPEC
	ret = __pkvm_create_mappings(addr, size, phys, prot, HYP_MODULE);
#else /* CONFIG_NVHE_GHOST_SPEC */
...
{
#ifdef CONFIG_NVHE_GHOST_SPEC
	int ret = __pkvm_create_mappings(va, PAGE_SIZE, phys, PAGE_HYP, HYP_ALLOCATOR_MAP);
#else /* CONFIG_NVHE_GHOST_SPEC */
...
#if defined(__KVM_NVHE_HYPERVISOR__) && defined(CONFIG_NVHE_GHOST_SIMPLIFIED_MODEL)
	casemate_model_step_tlbi_va(TLBI_vale2is, addr >> 12, (u64)level, 0ULL);
#endif /* defined(__KVM_NVHE_HYPERVISOR__) && defined(CONFIG_NVHE_GHOST_SIMPLIFIED_MODEL) */
	dsb(ish);
#if defined(__KVM_NVHE_HYPERVISOR__) && defined(CONFIG_NVHE_GHOST_SIMPLIFIED_MODEL)
