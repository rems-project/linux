...
static int pkvm_init_features_from_host(struct pkvm_hyp_vm *hyp_vm, const struct kvm *host_kvm)
{
...
static int init_pkvm_hyp_vm(struct kvm *host_kvm, struct pkvm_hyp_vm *hyp_vm,
			    int *last_ran, unsigned int nr_vcpus, pkvm_handle_t handle)
{
	...
	mmu->pgt = &hyp_vm->pgt;
	mmu->last_vcpu_ran = (int __percpu *)last_ran;
...
static int pkvm_vcpu_init_sve(struct pkvm_hyp_vcpu *hyp_vcpu, struct kvm_vcpu *host_vcpu)
{
	...
	if (pkvm_hyp_vcpu_is_protected(hyp_vcpu)) {
		...
		sve_state = hyp_alloc_account(sve_state_size, hyp_vm->host_kvm);
		if (!sve_state)
...
int __pkvm_init_vm(struct kvm *host_kvm, unsigned long pgd_hva)
{
	...
	ret = init_pkvm_hyp_vm(host_kvm, hyp_vm, last_ran, nr_vcpus, handle);
	if (ret)
...
int __pkvm_finalize_teardown_vm(pkvm_handle_t handle)
{
	...
	hyp_free_account((__force void *)hyp_vm->kvm.arch.mmu.last_vcpu_ran, host_kvm);
	hyp_free_account(hyp_vm, host_kvm);
	hyp_unpin_shared_mem(host_kvm, host_kvm + 1);
	return 0;
...
static bool pkvm_memrelinquish_call(struct pkvm_hyp_vcpu *hyp_vcpu, u64 *exit_code)
{
	struct kvm_vcpu *vcpu = &hyp_vcpu->vcpu;
	u64 ipa = smccc_get_arg1(vcpu);
