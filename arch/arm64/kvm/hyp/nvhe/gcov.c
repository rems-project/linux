...
#error "pKVM GCOV requires Clang."
#endif
 ...
 * them as initialisers, by placing them in the .init_array section.
 * Each initialiser calls llvm_gcov_init, below, with arguments specific to its
 * respective compilation unit. The arguments are two callbacks:
 *   - writeout makes a sequence of calls to the llvm_gcda_XXX functions, below,
 *   supplying the data collected by the instrumentation.
 *   - reset clears the coverage counters.
 *
 * On init, we store the callbacks and let them be driven by hypercalls, writing
 * data into a buffer shared with the kernel.
 *
 * Importantly, the profiling counters are updated irrespective of
 * initialisation; it merely registers entry points to pull and clear the data.
 * This gives us some freedom in arranging the init sequence.
 */
