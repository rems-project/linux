...
static struct maplet * get_maplet(struct glist_head *maplets_list)
{
...
static bool maplet_target_can_extend(struct maplet *lhs, u64 ia, u64 nr_pages, struct maplet_target *rhs)
{
...
static void hyp_put_mapletptr(void *d)
{
	struct maplet *m = *(struct maplet**)d;
	hyp_put_maplet(m, 0);
}

int gp_put_mapping(gp_stream_t *out, mapping *mapp, u64 indent)
{
...
static bool __maplet_target_eq(struct maplet_target t1, struct maplet_target t2, bool check_oa_range_start, bool check_oa_range_size, bool check_attrs) {
	if (t1.kind != t2.kind)
		return false;
...
static bool maplet_target_eq(struct maplet_target t1, struct maplet_target t2)
{
	return __maplet_target_eq(t1, t2, true, true, true);
}

static bool maplet_target_eq_nonattr(struct maplet_target t1, struct maplet_target t2)
{
	return __maplet_target_eq(t1, t2, true, true, false);
}

static bool maplet_target_eq_nonend(struct maplet_target t1, struct maplet_target t2)
{
...
static void check_interpret_equals(struct glist_head head1, struct glist_head head2, u64 i)
{
	struct glist_node *pos1, *pos2;
	struct maplet *m1, *m2;
...
static void maplet_shift(struct maplet *m1, u64 shift)
{
...
static void maplet_target_reduce(struct maplet_target *t, u64 shift)
{
	switch (t->kind) {
	case MAPLET_MAPPED:
...
static struct glist_head mapping_copy_except_absent(struct glist_head head)
{
	struct glist_node *pos2;
	struct maplet *m2;
...
static bool maplet_in_domain(u64 ia, struct maplet *m)
{
	if (m == NULL)
		return false;
...
static bool __mapping_lookup(u64 virt, struct glist_head head, struct maplet **p)
{
	struct glist_node *pos1;
	struct maplet *m1;
