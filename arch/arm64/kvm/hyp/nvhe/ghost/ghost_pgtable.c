...
static void _dump_pgtable(u64 *pgd, u8 level, u8 indent)
{
	u32 idx;
	if (pgd) {
		// dump this page
...
static enum maplet_owner_annotation __parse_annot(u64 pte)
{
...
static inline enum maplet_page_state convert_page_state(enum pkvm_page_state page_state)
{
	...
	case PKVM_MODULE_SHARED_OWNED_PAGE:
	case PKVM_PAGE_RESTRICTED_PROT:
	case PKVM_MMIO:
	case PKVM_ACCEPT_MODULE_OWNED:
