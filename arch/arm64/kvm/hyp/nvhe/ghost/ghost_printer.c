...
static int __putc(gp_stream_t *out, char c)
{
	switch (out->kind) {
	case GP_STREAM_UART:
		hyp_putc(c);
...
static int __puts(gp_stream_t *out, char *s)
{
	while (*s)
		TRY_PUT(*s++);
...
static int __puti(gp_stream_t *out, int width, char c)
{
	while (width--)
		TRY_PUT(c);
...
static int __putn(gp_stream_t *out, u64 n)
{
	char digits[20] = {0};
	int i = 0;
...
static int __putx(gp_stream_t *out, u32 x)
{
	x &= 0xf;
	if (x <= 9)
		x += '0';
...
static int __putxn(gp_stream_t *out, u64 x, u32 n)
{
	int i = n >> 2;
...
static int put_char(gp_stream_t *out, char **p, int arg)
{
	/*
...
static int put_bool(gp_stream_t *out, char **p, int arg)
{
	bool b = (bool)arg;
	if (b)
...
static int put_str(gp_stream_t *out, char **p, int width, char *arg)
{
	int n;
...
static int put_decimal(gp_stream_t *out, char **p, u64 width, _Bool is_signed, enum arg_length len, u64 x)
{
	int n;
	char sign = 0;
...
static int put_hex(gp_stream_t *out, char **p, u64 width, enum arg_length len, u64 x)
{
	// pad the left-hand side with . up to width
	int n = 2 + (len >> 2);
	if (n < width)
...
static int put_raw_ptr(gp_stream_t *out, char **p, void *arg)
{
	u64 x = (u64)arg;
	return __putxn(out, x, 64);
}

static int put_kern_ptr(gp_stream_t *out, char **p, void *arg)
{
	u64 x = (u64)arg;
...
static int put_phys_ptr(gp_stream_t *out, char **p, void *arg)
{
	u64 x = (u64)arg;
	// kernel pointers are printed as RAW/HYP_VA
	TRY(__putxn(out, x, 64));
...
static int put_indent(gp_stream_t *out, char **p, u64 arg)
{
	while (arg--)
		TRY_PUT(' ');
	return 0;
...
static bool __matches(char *p, const char *kind)
{
...
static int put_ghost_obj(gp_stream_t *out, char **p, u64 arg0, u64 arg1)
{
	// %g(KIND)
 ...
 */
static int slice_off_width(char **p)
{
	int d = 0;
 ...
 */
static enum arg_length slice_off_length(char **p)
{
	if (__matches(*p, "hh")) {
		*p += 2;
		return LENGTH_hh;
