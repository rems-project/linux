...
static void sort_mapping_reqs_virt(struct mapping_reqs *mapping_reqs)
{
	sort(&mapping_reqs->m, mapping_reqs->count, sizeof(struct mapping_req), mapping_req_compare_virt, NULL);
}

#if 0
static void sort_mapping_reqs_phys(struct mapping_reqs *mapping_reqs)
{
...
/* print mapping_reqs */
static void hyp_put_prot(enum kvm_pgtable_prot prot)
{
...
static void hyp_put_mapping_req_kind(enum mapping_req_kind kind)
{
	switch (kind) {
...
static void hyp_put_mapping_req(struct mapping_req *map)
{
...
static void hyp_put_mapping_reqs(struct mapping_reqs *mapping_reqs)
{
	u64 i;
...
static void extend_mapping_reqs_virt(struct mapping_reqs *mapping_reqs, enum mapping_req_kind kind, u64 cpu, char *doc, void *virt_from, void *virt_to, enum kvm_pgtable_prot prot)
{
	u64 virt_from_aligned, virt_to_aligned;
	u64 size;
	phys_addr_t phys;
...
static void extend_mapping_reqs_image_idmap(struct mapping_reqs *mapping_reqs, enum mapping_req_kind kind, u64 cpu, char *doc, void *virt_from, void *virt_to, enum kvm_pgtable_prot prot)
{
	u64 virt_from_aligned, virt_to_aligned;
	u64 size;
	phys_addr_t phys;
...
static void extend_mapping_reqs_vmemmap(struct mapping_reqs *mapping_reqs, enum mapping_req_kind kind, u64 cpu, char *doc, void *virt_from, void *virt_to, phys_addr_t phys, enum kvm_pgtable_prot prot)
{
	u64 virt_from_aligned, virt_to_aligned;
	u64 size;
	if (mapping_reqs->count >= MAX_MAPPING_REQS)
...
// TODO: do we still need this? (this is copied from pgtable.c)
#if 0
...
static struct maplet_attributes attrs_from_req(struct mapping_req *r)
{
...
static mapping interpret_mapping_reqs(void)
{
...
void ghost_check_hyp_mapping_reqs(struct kvm_pgtable *pg, bool noisy)
{
	...
	mapping mapping_pkvm = ghost_record_pgtable(pg, NULL, NULL, "check_hyp_mapping_reqs pgd", 2);
